...
public partial class Admin_Ajax_ExhibitorSort : AjaxPageAdmin
{
    ...
    {
        int exhibitorId = 0, previewId = 0, nextId = 0, siteId = 0;

        int.TryParse( QS( "sid" ), out siteId );
        if( siteId == 0 )
        {
            RenderString( "站点不存在！" );
        }

        AW_Site_bean site = new AW_Site_dao().funcGetSiteInfo( siteId );
        if( site == null )
        {
            RenderString( "站点不存在！" );
        }
        ...
        if( exhibitorId > 0 && ( previewId > 0 || nextId > 0 ) )
        {
            ...
            {
                if( dao.funcSortExhibitor( siteId, exhibitor, preview, next ) )
                {
