    ...
    tinymce.create('tinymce.plugins.MediaPlugin', {
        init: function(ed, url) {
            ...
            ed.onPostProcess.add(function(ed, o) {
                ...
                o.content = o.content.replace(/_flash_/g, '&');
            });
        ...
        _buildObj: function(o, n) {
            ...
            each(p, function(v, k) {
                if (!/^(width|height|codebase|classid|_cx|_cy)$/.test(k)) {
                    ...
                    if (v && o.type != 'flash-vidio')
                        dom.add(ob, 'span', { mce_name: 'param', name: k, '_mce_value': v });
                }
            });

            if (!stc) {
                if (o.type == 'flash-vidio') {
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'param', name: 'src', _mce_value: '/img/FLVPlayer_Progressive.swf' }, p));
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'param', name: 'movie', _mce_value: '/img/FLVPlayer_Progressive.swf' }, p));
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'param', name: 'salign', _mce_value: 'lt' }, p));
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'param', name: 'quality', _mce_value: 'high' }, p));
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'param', name: 'scale', _mce_value: 'noscale' }, p));
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'param', name: 'FlashVars', _mce_value: '_flash_MM_ComponentVersion=1_flash_skinName=/img/Halo_Skin_3_flash_autoPlay=false_flash_autoRewind=false_flash_streamName=' + p.src }, p));
                    var temp = p.src;
                    p.src = "/img/FLVPlayer_Progressive.swf";
                    dom.add(ob, 'span', tinymce.extend({ mce_name: 'embed', flashvars: '_flash_MM_ComponentVersion=1_flash_skinName=/img/Halo_Skin_3_flash_autoPlay=false_flash_autoRewind=false_flash_streamName=' + temp, quality: 'high', scale: 'noscale', salign: 'lt', type: 'application/x-shockwave-flash', pluginspage: 'http://www.macromedia.com/go/getflashplayer' }, p));
                    p.src = temp;
                } else {
        ...
        _createImg: function(cl, n) {
            ...
            //CZM:mceItemFlashVideo
            if (cl == 'mceItemFlashVideo') {
                im = dom.create('img', {
                ...
                pa['width'] = dom.getAttrib(n, 'width');
                pa['height'] = dom.getAttrib(n, 'height');

                n = dom.select('.mceItemEmbed', n)[0];
                if (n) {
                    pa['src'] = dom.getAttrib(n, 'FlashVars');
                    pa['src'] = pa['src'].substr(pa['src'].indexOf('streamName=') + 11);
                }
