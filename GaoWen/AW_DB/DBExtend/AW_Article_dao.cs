...
namespace AnyWeb.AW_DL
{
    public partial class AW_Article_dao
    {
        ...
        /// <returns>0:多篇；否则返回文章编号</returns>
        public int funcHasOnlyArticle(AW_Column_bean column) {
            this.propSelect = "a.fdArtiID";
            this.propTableApp = " a INNER JOIN AW_Column c ON a.fdArtiColumnID = c.fdColuID";
            this.propOrder = "ORDER BY a.fdArtiSort DESC,fdArtiID DESC";
            this.propTopCount = 2;
            this.propWhere = string.Format("fdColuID={0}", column.fdColuID);
            if (column.Children != null)
            {
                this.propWhere += string.Format(" OR fdColuParentID={0}", column.fdColuID);
            }
            DataSet ds = this.funcCommon();
