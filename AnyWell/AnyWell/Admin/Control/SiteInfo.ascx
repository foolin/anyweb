        ...
        <ul>
            ...
            <li class="template"><strong>首页模板</strong>:
                <%if( CurrentSite.IndexTemplate == null )
                  {%>
                <input type="text" title="新建模板" value="未设置" readonly="readonly" onclick="window.open('../Template/TemplateAdd.aspx?site=true&sid=<%=CurrentSite.fdSiteID %>&type=4', 'template');" />
                <%}
                  else
                  { %>
                <input type="text" title="修改模板" value="<%=CurrentSite.IndexTemplate.fdTempName %>"
                    readonly="readonly" onclick="window.open('../Template/TemplateEdit.aspx?sid=<%=CurrentSite.fdSiteID %>&id=<%=CurrentSite.fdSiteIndexTemplateID %>', 'template');" />
                <%} %>
            ...
            <li class="template"><strong>栏目模板</strong>:
                ...
                <input type="text" title="修改模板" value="<%=CurrentSite.ColumnTemplate.fdTempName %>"
                    readonly="readonly" onclick="window.open('../Template/TemplateEdit.aspx?sid=<%=CurrentSite.fdSiteID %>&id=<%=CurrentSite.fdSiteColumnTemplateID %>', 'template');" />
                <%} %>
            ...
            <li class="template"><strong>内页模板</strong>:
                <%if( CurrentSite.ContentTemplate == null )
                  {%>
                <input type="text" title="新建模板" value="未设置" readonly="readonly" onclick="window.open('../Template/TemplateAdd.aspx?site=true&sid=<%=CurrentSite.fdSiteID %>&type=2', 'template');" />
                <%}
                  else
                  { %>
