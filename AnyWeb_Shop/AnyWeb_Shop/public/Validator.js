var Validator = {
	...
	ValidateForm : function (form){
		this.Results = [];
		var frm = form || document.forms[document.forms.length-1];
		var errs = "";
	...
	Validate : function(ctl, showmsg){
		...
		if( validated == true){
			...
			var result = new Validator.Result(ctl, isok, msg);
		...
		switch(this.Style){
			default:{
				...
				if( msgbox){
					...
					msgbox.innerText = msgobj.message;		
				}
				else{
		...
		for( i = 0; i<ctls.length; i++){
			...
			&& !ctls[i].nocheck){
				if( ctls[i].onclick){
					...
					ctls[i].onclick = function(){return Validator.ValidateForm(document.forms[document.forms.length-1]);};
				}
			}
