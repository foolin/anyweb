...
namespace AnyWell.AW_DL
{
	public partial class AW_Resume_dao
	{
        ...
        {
            this.propSelect = "fdResuID,fdResuUserID,fdResuDegree";
            this.propWhere = "fdResuID=" + resuId;
            DataSet ds = this.funcCommon();
            if( ds.Tables[ 0 ].Rows.Count > 0 )
            {
        ...
        /// <returns></returns>
        public int funcUpdateResumeDegree( int resuId )
        {
            string sql = string.Format( "UPDATE AW_Resume SET fdResuDegree=(SELECT MAX(fdEducDegree) FROM AW_Education WHERE fdEducResuID={0}) WHERE fdResuID={0}", resuId );
            return this.funcExecute( sql );
        }
        ...
        //        this.funcAddParam( "@agefrom", DateTime.Parse( string.Format( "{0}-1-1", DateTime.Now.Year - agefrom + 1 ) ) );
        //    }
        //    if( ageto != 0 )
        //    {
        ...
        public List<AW_Resume_bean> funcSearchResume( int resuId, int type, string key, int workfrom, int workto, int edufrom, int eduto, int agefrom, int ageto, int sex, int saleryfrom, int saleryto, int[] major, int[] address, int[] industry, DateTime updatefrom, DateTime updateto, int pageIndex, int pageSize, out int recordCount )
        {
            ...
            if( edufrom != 0 )
            {
                this.propWhere += " AND fdResuDegree>=" + edufrom;
            }
            if( eduto != 0 )
            {
                this.propWhere += " AND fdResuDegree<=" + eduto;
            }
            if( agefrom != 0 )
            {
                this.propWhere += " AND fdResuBirthday<@agefrom";
